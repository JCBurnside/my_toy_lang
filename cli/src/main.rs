use std::{io::BufReader, fs::File, collections::HashMap, path::PathBuf};

use clap::Parser;
use inkwell::{targets::{Target, InitializationConfig}, context::Context, OptimizationLevel, types::BasicType};

use compiler::types::{ResolvedType, self};
...
fn main() {
    ...
    if !args.file.exists() {
        println!("input file {} does not exist", args.file.as_path().to_string_lossy());
        return;
    }
    ...
    fwd_decl.insert("print_str".to_owned(), ResolvedType::Function { arg: Box::new(types::STR), returns: Box::new(types::UNIT) });

    let program = compiler::from_file(&args.file,&ctx,fwd_decl);
    match program {
        ...
        Ok(module) => {
            if args.run {
                ...
                // TODO: Jit redirects.
                unsafe {
                    jit.get_function::<unsafe extern "C" fn(())->()>("main").unwrap().call(());
                }
