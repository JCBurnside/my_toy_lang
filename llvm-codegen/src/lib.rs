...
use inkwell::{context::Context, execution_engine::{ExecutionEngine, JitFunction, UnsafeFunctionPointer}, module::Module, object_file::ObjectFile, targets::{CodeModel, InitializationConfig, Target, TargetMachine}};
use multimap::MultiMap;
use type_resolver::TypeResolver;

//TODO! move doing the forward declares into here.  
pub fn compile_file(mut ast:compiler::typed_ast::FileTyped,file:PathBuf,write_llvm_to:Option<PathBuf>, fwd_decls:HashMap<String,ResolvedType>) {
...
pub fn create_jit_runtime() -> JitEngine {
    ...
    JitEngine::new(ctx,|ctx| {
        ...
        let gen = CodeGen::with_module(ctx, module, type_resolver, HashMap::new(), HashMap::new(), MultiMap::new(), target_machine.get_target_data());
        BoundItems{
            generator:gen,
            engine : ee,
            modules:Vec::new(),
...
impl JitEngine {
    pub fn add_declarations(&mut self, mut decls : Vec<TypedDeclaration>) {
        ...
        self.with_dependent_mut(|ctx,BoundItems {generator:gen, modules,..}| {
