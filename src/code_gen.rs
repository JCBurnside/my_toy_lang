...
impl<'ctx, 'module> CodeGen<'ctx, 'module> {
    ...
    pub fn compile_program(
        self,
    ) -> Result<Vec<Box<dyn inkwell::values::AnyValue<'ctx>>>, &'static str> {
        let builder = self.ctx.create_builder();
        self.ast
            .iter()
            .map(|expr| self.compile_expr(&builder, expr))
            .collect()
    }
    ...
    fn compile_expr(
        ...
        builder: &Builder,
        expr: &Expr,
    ) -> Result<Box<dyn inkwell::values::AnyValue<'ctx>>, &'static str> {
        todo!()
    }

    fn compile_function(&mut self, builder: &Builder) -> Result<FunctionValue<'ctx>, &'static str> {
        todo!();
        // if let Some(Expr::Declaration {
        //     ident,
        //     ty,
        //     is_op,
        //     args,
